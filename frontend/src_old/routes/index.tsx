  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden min-h-[90vh] flex items-center">
        <HeroVideoBackground
          videoSrc="/videos/xyz.mp4"
          posterSrc={IMG("perfume-hero-luxury", 1920, 1200)}
          parallax={parallax}
        />
        {/* Decorative elements */}
        ...
        <div className="mx-auto max-w-7xl px-4 py-20 md:px-6 w-full">
          <div className="max-w-2xl">
            <div className="text-pop-in flex items-center gap-2 text-gold text-sm tracking-[0.3em] uppercase" style={{ animationDelay: "0ms" }}>
              <Crown className="h-4 w-4" />
              <span>Est. 2024 · Luxury Fragrances</span>
            </div>
            <h1 className="text-pop-in mt-6 font-serif text-6xl leading-[1.05] text-primary md:text-8xl" style={{ animationDelay: "120ms" }}>
              Wear Your<br />
              <span className="text-gold">Royalty</span>
            </h1>
            <p className="text-pop-in mt-6 max-w-lg text-lg text-foreground/70 leading-relaxed" style={{ animationDelay: "240ms" }}>
            ...
            <div className="text-pop-in mt-10 flex flex-wrap gap-4" style={{ animationDelay: "360ms" }}>
              <Link to="/shop" className="group relative inline-flex items-center gap-2 overflow-hidden rounded-full bg-gold px-8 py-4 text-sm font-medium text-gold-foreground shadow-lg shadow-gold/20 transition-all hover:shadow-gold/40 hover:scale-105">
                <span className="shine-sweep pointer-events-none absolute inset-y-0 left-0 w-1/3 bg-white/30" />
                <span className="relative z-10 flex items-center gap-2">Explore Collection <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span>
              </Link>
              <Link to="/about" className="inline-flex items-center gap-2 rounded-full border border-gold/30 px-8 py-4 text-sm text-foreground hover:bg-gold/10 transition-all hover:scale-105">
            ...
            <div className="mt-12 flex flex-wrap gap-8 text-sm">
              {[
                ...
                <div key={item.text} className="text-pop-in flex items-center gap-2 text-foreground/60" style={{ animationDelay: `${480 + i * 100}ms` }}>
      ...
      <section className="border-y border-gold/10 bg-card/50">
        <div className="marquee-viewport py-6">
          <div className="marquee-track-slow gap-16 pr-16">
            {Array.from({ length: 2 }).map((_, row) => (
              <div key={row} className="flex shrink-0 items-center gap-12 pl-16" aria-hidden={row === 1}>
                {["Premium Ingredients", "Handcrafted in India", "Cruelty-Free", "Luxury Packaging", "100% Satisfaction"].map((t) => (
                  <span key={t} className="flex items-center gap-2 whitespace-nowrap text-xs uppercase tracking-[0.2em] text-muted-foreground">
      ...
      <section className="mx-auto max-w-7xl px-4 py-24 md:px-6">
        ...
        <div className="relative mt-14">
          <button
            ...
            className="absolute left-0 top-1/2 -translate-y-1/2 z-10 hidden md:grid h-10 w-10 place-items-center rounded-full bg-background/80 border border-border shadow-lg transition-all hover:scale-110 hover:bg-gold hover:text-gold-foreground"
          >
            <ChevronLeft className="h-5 w-5" />
          </button>
          <button
            onClick={() => scrollCategories("right")}
            className="absolute right-0 top-1/2 -translate-y-1/2 z-10 hidden md:grid h-10 w-10 place-items-center rounded-full bg-background/80 border border-border shadow-lg transition-all hover:scale-110 hover:bg-gold hover:text-gold-foreground"
          >
          ...
          >
            {displayCategories.map((c, i) => (
              <Reveal key={c.name} delay={i * 80} className="snap-start shrink-0 w-[75vw] sm:w-[45vw] md:w-[30vw] lg:w-[22vw] max-w-[280px]">
                <Link to="/shop" className="group hover-lift relative block overflow-hidden rounded-2xl bg-card border border-border/40 aspect-[3/4]">
      ...
      <section className="bg-card/30 py-24">
        <div className="mx-auto max-w-7xl px-4 md:px-6">
          ...
          <div className="mt-10 flex justify-center gap-8 text-sm">
            {(["Best Seller", "New", "Featured"] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`border-b-2 pb-1.5 transition-all duration-300 ${tab === t ? "border-gold text-gold" : "border-transparent text-muted-foreground hover:text-foreground hover:border-gold/30"}`}
              >
          ...
          <div className="mt-12 text-center">
            <Link to="/shop" className="group inline-flex items-center gap-2 rounded-full border border-gold/30 px-8 py-3.5 text-sm text-foreground transition-all hover:bg-gold hover:text-gold-foreground hover:-translate-y-0.5">
      ...
      <section className="relative overflow-hidden">
        <img src={IMG("perfume-story", 1920, 900)} alt="" className="h-[70vh] w-full scale-110 object-cover animate-float" style={{ animationDuration: "14s" }} />
        ...
        <div className="absolute inset-0 flex items-center justify-center p-6">
          <Reveal className="max-w-2xl text-center">
            ...
            <button className="mt-8 inline-flex items-center gap-2 rounded-full border border-gold/30 px-6 py-3 text-sm text-foreground transition-all hover:bg-gold hover:text-gold-foreground hover:scale-105">
      ...
      <section className="mx-auto max-w-7xl px-4 py-24 md:px-6">
        ...
        <div className="mt-16 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <Reveal key={b.title} delay={i * 80} className="hover-lift text-center group">
              <div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-gold/10 text-gold transition-all duration-500 group-hover:bg-gold group-hover:text-gold-foreground group-hover:rotate-6">
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6">
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-3">
            {testimonials.map((t, i) => (
              <Reveal key={t.name} delay={i * 80} className="hover-lift rounded-2xl bg-card border border-border/40 p-8 hover:border-gold/20 hover:shadow-lg hover:shadow-gold/5 transition-all">
      ...
      <section className="mx-auto max-w-7xl px-4 py-24 md:px-6">
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-6">
          {Array.from({ length: 6 }).map((_, i) => (
            <a key={i} href="#" className="group hover-lift relative aspect-square overflow-hidden rounded-xl">
      ...
      {/* NEWSLETTER */}
      <section className="relative overflow-hidden bg-gradient-to-b from-background to-card/50 border-t border-gold/10">
        <div className="aurora-blob absolute left-1/2 top-0 h-[300px] w-[500px] -translate-x-1/2 rounded-full bg-burgundy/15" />
        <div className="relative mx-auto max-w-3xl px-4 py-24 text-center md:px-6">
          ...
          <form className="mx-auto mt-10 flex max-w-md gap-3" onSubmit={(e) => e.preventDefault()}>
            <input type="email" placeholder="Your email address" className="flex-1 rounded-full border border-gold/20 bg-background px-6 py-3.5 text-sm text-foreground placeholder:text-muted-foreground/50 outline-none transition-colors focus:border-gold" />
            <button className="group relative overflow-hidden rounded-full bg-gold px-8 py-3.5 text-sm font-medium text-gold-foreground transition-all hover:opacity-90">
              <span className="shine-sweep pointer-events-none absolute inset-y-0 left-0 w-1/3 bg-white/30" />
